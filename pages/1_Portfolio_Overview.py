...
def render_summary_card(title, value, meta=""):
    st.markdown(
        f"""
...
def render_asset_card(title, subtitle, value, pnl_text="", pnl_positive=None):
...
def get_coin_metrics(portfolio, usdczk):
    metrics = {}
    unavailable_prices = []
...
with st.expander(f"₿ Kryptoměny · Celkem investováno {fmt_czk(crypto_total_cost_czk)}", expanded=False):
    ...
    for coin in TRACKED_COINS:
        ...
        render_asset_card(
            ...
            pnl_text=(f"{m['pnl_pct']:+.2f}%" if m["pnl_pct"] is not None else "Cena nedostupná"),
...
with st.expander(f"📈 XTB + My Trades · Celkem investováno {fmt_czk(invest_total_cost_czk)}", expanded=False):
    ...
    with x3:
        xtb_pct = ((invest_total_pnl_czk_base / invest_total_cost_czk_base) * 100 if invest_total_cost_czk_base > 0 else 0.0)
        render_summary_card("Celkem XTB", fmt_czk(invest_total_value_czk_base), f"Výsledek {xtb_pct:+.2f}%")
...
with st.expander(f"🏠 Investown · Celkem investováno {fmt_czk(investown_total_cost_czk)}", expanded=False):
    ...
    with j2:
        render_summary_card("Konzervativní hodnota", fmt_czk(investown_total_value_czk), "Počítáno bez penalizace jistiny")

    j3, j4 = st.columns(2)
    with j3:
        render_summary_card("Výnos z úroků", fmt_czk(investown_total_pnl_czk), "Zobrazen pouze úrokový výnos")
    with j4:
        render_summary_card("Počet projektů", str(investown_projects_count), f"Vyplaceno {fmt_czk(investown_paid_out_czk)}")
    ...
    if not INVESTOWN_AVAILABLE:
        ...
        render_asset_card(
            title="Investown přehled",
            subtitle="Souhrn investovaných prostředků, konzervativní hodnoty a úrokového výnosu",
            value=fmt_czk(investown_total_value_czk),
            pnl_text=fmt_czk(investown_total_pnl_czk),
